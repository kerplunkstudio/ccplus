...
describe('stream-query session status regression', () => {
  ...
  describe('Bug Fix 4: non-git workspaces must not crash with --worktree error', () => {
    /**
     * Root cause: worktreeEnabled was determined from config/workflow and passed to the SDK
     * without checking whether the workspace is actually a git repository. The SDK then
     * crashed with "Can only use --worktree in a git repository".
     *
     * Fix: before calling ensureWorkspaceUpToDate, run `git rev-parse --is-inside-work-tree`.
     * If it fails (non-git workspace), set worktreeEnabled = false so extraArgs is not passed.
     */
    it('does NOT pass extraArgs.worktree when workspace is not a git repository', async () => {
      mockCaptain.notifySessionComplete.mockImplementation(() => {});
      // execAsync rejects for all calls (non-git workspace) — this is the default mock
      mockExecAsync.mockRejectedValue(new Error('not a git repository'));
      mockQuery.mockReturnValue(createResultQuery(false));

      const sessionId = 'non-git-workspace-test';
      const callbacks = { ...BASE_CALLBACKS, onText: vi.fn(), onComplete: vi.fn(), onError: vi.fn() };

      sdkSession.submitQuery(sessionId, 'Hello', '/tmp/non-git-workspace', callbacks);
      await new Promise((r) => setTimeout(r, 200));

      // Query must have been called (session doesn't crash)
      expect(mockQuery).toHaveBeenCalled();

      // extraArgs must NOT include worktree
      const queryOptions = mockQuery.mock.calls[0][0].options;
      expect(queryOptions.extraArgs).toBeUndefined();
    });

    it('passes extraArgs.worktree when workspace IS a git repository', async () => {
      mockCaptain.notifySessionComplete.mockImplementation(() => {});
      // execAsync resolves for git check (is a git repo), but rejects for ensureWorkspaceUpToDate internals
      mockExecAsync.mockResolvedValueOnce({ stdout: 'true\n', stderr: '' });
      mockQuery.mockReturnValue(createResultQuery(false));

      const sessionId = 'git-workspace-test';
      const callbacks = { ...BASE_CALLBACKS, onText: vi.fn(), onComplete: vi.fn(), onError: vi.fn() };

      sdkSession.submitQuery(sessionId, 'Hello', '/tmp/git-workspace', callbacks);
      await new Promise((r) => setTimeout(r, 200));

      expect(mockQuery).toHaveBeenCalled();

      // extraArgs should include worktree: null
      const queryOptions = mockQuery.mock.calls[0][0].options;
      expect(queryOptions.extraArgs).toEqual({ worktree: null });
    });

    it('session completes successfully (not "failed") when workspace is not a git repository', async () => {
      mockCaptain.notifySessionComplete.mockImplementation(() => {});
      mockExecAsync.mockRejectedValue(new Error('not a git repository'));
      mockQuery.mockReturnValue(createResultQuery(false));

      const sessionId = 'non-git-status-test';
      const callbacks = { ...BASE_CALLBACKS, onText: vi.fn(), onComplete: vi.fn(), onError: vi.fn() };
